...
import {
  Paintbrush, Droplets, Eraser, Undo2, Trash2, Download,
...
export default function ColorPage({ params }: { params: Promise<{ purchaseId: string }> }) {
  ...
  // Convert client coords → canvas pixel coords (accounting for zoom + CSS scale)
  ...
  const handlePointerMove = useCallback(
    (e: React.MouseEvent | React.TouchEvent) => {
      e.preventDefault();
      // Update custom cursor position relative to the canvas wrapper
      const rect = canvasRef.current?.getBoundingClientRect();
      if (rect && cursorRef.current) {
        const src = "touches" in e ? (e as React.TouchEvent).touches[0] : (e as React.MouseEvent);
        if (src) {
          cursorRef.current.style.left = src.clientX - rect.left + "px";
          cursorRef.current.style.top  = src.clientY - rect.top  + "px";
        }
      }
    ...
    <>
      ...
      >
        ...
        <div className="relative z-20 bg-cream border-b border-border-muted px-4 py-2.5 flex items-center gap-3 shrink-0">
          ...
          <span className={`text-[11px] transition-opacity duration-300 ${saved ? "opacity-100 text-[#258635]" : "opacity-0"}`}>
            Saved
          </span>

          {/* Zoom controls */}
          <div className="hidden md:flex items-center gap-1 bg-[#EDEBE6] rounded-full px-1 py-1">
            <button
              onClick={zoomOut}
          ...
          <button
            ...
            className="w-8 h-8 rounded-full bg-[#EDEBE6] flex items-center justify-center hover:bg-card-hover transition-colors"
            ...
            <div>
              ...
              <label className="flex items-center gap-2 mt-3 cursor-pointer">
                <input
                  ...
                  className="sr-only"
                />
                <div className="w-7 h-7 rounded-full border-2 border-dashed border-ink-muted flex items-center justify-center text-[10px] text-ink-muted">
                  +
                </div>
          ...
          >
            ...
            <div
              style={{
                ...
                // Give enough room so scrollbars appear when zoomed beyond viewport
        ...
        <div className="md:hidden bg-cream border-t border-border-muted px-3 py-2 flex items-center gap-2 overflow-x-auto shrink-0">
          ...
          <button
            onClick={zoomReset}
            className="text-[11px] font-semibold text-ink shrink-0 w-10 text-center"
          >
            {Math.round(zoom * 100)}%
          </button>
          <button
          ...
          <button
            onClick={toggleFullscreen}
            className="flex items-center justify-center w-9 h-9 rounded-xl bg-[#EDEBE6] shrink-0 ml-auto"
          >
