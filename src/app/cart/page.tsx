        ...
        <div className="px-6 py-5">
          ...
          {step === "method" && (
            <div className="space-y-3">
              ...
              <button
                onClick={() => { if (method === "card") setStep("details"); }}
                disabled={!email || method !== "card"}
              ...
              >
                {method === "card" ? "Continue to Card Details" : "Coming Soon"}
              </button>
            </div>
          )}
...
export default function CartPage() {
  ...
  return (
    <>
        ...
        <div className="max-w-5xl mx-auto px-6 py-10">
          ...
          {items.length === 0 && (
            <div className="flex flex-col items-center justify-center py-32 text-center">
              ...
              <p className="text-sm text-ink-muted mb-6">Add some products to get started.</p>
              <a
                href="/all"
            ...
            <div className="flex flex-col lg:flex-row gap-8">
              ...
              <div className="flex-1 space-y-4">
                {items.map(({ product }) => {
                  ...
                  return (
                    <div key={product.id} className="flex gap-4 p-4 bg-white rounded-2xl border border-border-muted">
                      <a href={`/product/${product.id}`} className="shrink-0">
                        {/* eslint-disable-next-line @next/next/no-img-element */}
                        <img src={product.image} alt={product.title} className="w-20 h-20 rounded-xl object-cover bg-card-hover" />
                      </a>
                      <div className="flex-1 min-w-0">
              ...
              <div className="lg:w-80 shrink-0">
                <div className="bg-white border border-border-muted rounded-2xl p-6 sticky top-24">
                  ...
                  <a href="/all" className="block text-center text-xs text-ink-muted hover:text-ink underline underline-offset-2 transition-colors duration-200">
                    Continue shopping
                  </a>
                </div>
              </div>
      ...
      {showAuthModal && (
        <AuthModal onClose={() => setShowAuthModal(false)} message="You need to sign in to proceed to checkout." />
      )}
    </>
  );
}
