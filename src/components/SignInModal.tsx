...
/**
 * Sign-in modal — overlay version of /signin/page.tsx. Opened from
 * the Navbar's "Sign in" buttons instead of navigating away. On
 * successful auth, just closes the modal — the AuthContext picks up
 * the new session and any auth-gated UI re-renders in place.
 *
 * Keeps the standalone /signin route intact for deep links, password-
 * reset return URLs, and JS-disabled visitors.
 */
...
export default function SignInModal({ open, onClose }: SignInModalProps) {
  ...
  // Reset transient state every time the modal opens. Otherwise a
  // previous failed attempt would leak "wrong password" into the
  // next open.
  useEffect(() => {
    if (open) {
      setError("");
  ...
  // Lock body scroll while the modal is open so the backdrop click
  // target stays put when the underlying page is long.
  useEffect(() => {
    if (!open) return;
  ...
  async function handleSignIn(e: React.FormEvent) {
    ...
    // Success — close the modal. AuthContext re-renders the page.
    onClose();
    setEmail("");
    setPassword("");
  }
  ...
  return (
    <div
      className="fixed inset-0 z-[60] bg-black/40 flex items-center justify-center p-4 animate-[modalFadeIn_180ms_ease-out]"
      onClick={onClose}
      role="dialog"
      aria-modal="true"
    ...
    >
      <div
        className="relative w-full max-w-md bg-white rounded-2xl border border-border-muted shadow-2xl animate-[modalPop_220ms_cubic-bezier(0.34,1.56,0.64,1)]"
        onClick={(e) => e.stopPropagation()}
      >
        <button
          onClick={onClose}
          className="absolute right-4 top-4 w-8 h-8 rounded-full hover:bg-card-hover flex items-center justify-center text-ink-muted hover:text-ink transition-colors"
        ...
        <div className="p-7 sm:p-8">
          ...
          <h1 id="signin-modal-title" className="font-serif text-2xl font-semibold text-ink mb-1 text-center">
            Welcome back
          </h1>
          ...
          <form onSubmit={handleSignIn} className="flex flex-col gap-3">
            ...
            <div className="flex justify-end">
              <a href="/signin" className="text-xs text-ink-muted hover:text-ink underline">
                Forgot password?
              </a>
            </div>
